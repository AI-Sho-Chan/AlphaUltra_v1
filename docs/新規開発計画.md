...
```
## US拡張（EDGAR-first）
目的：JPと同一規範でUSも一次情報ベース化。T+1（US現地寄り）で整合。

US-T0: 方針
- 時刻はET基準。8‑K/10‑Q/10‑K/IRをソース。リーケージ禁止。

US-T1: 取込
- scripts/edgar_ingest.py --days 3650 --forms 8-K 10-Q 10-K
- 出力: data/us/raw/edgar_events.parquet (ticker, announce_dt_et, form, item_codes, text)

US-T2: 価格
- data/us/proc/prices.parquet を生成（Yahoo失敗はStooq補完）。

US-T3: 寄り合わせ＋ラベル
- eff_date_us（次営業日寄り）を付与。
- y_2x(252d), y_10x(1260d), 相対(1M/3M/12M)
- 出力: data/us/proc/us_panel.parquet, data/us/proc/us_targets.parquet

US-T4: 健全性
- 欠損・分布・T+1整合の検査（healthcheckスクリプト）。

US-T5: CV
- Purged/Embargo (purge=20d, embargo=5d)。クラスタPurgeは gics_sub_industry 優先。

US-T6: 学習＋DSR
- configs/us_model.yaml で LightGBM。コスト控除: 往復10bps。

US-T7: 校正＋閾値
- Isotonic → PPV‑Coverage で PPV≥80% の最小θ*を決定し thresholds.yaml に固定。
