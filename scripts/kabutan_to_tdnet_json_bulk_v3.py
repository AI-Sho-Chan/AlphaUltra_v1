...
for fp in files:
    ...
    if len(PRICE_KEYS & low) >= 3:  # 価格表はスキップ
        continue
    ...
    folder_d = find_date_in_path(fp)
    if folder_d:
        fill_dt = pd.to_datetime(folder_d+" 15:00:00")
        if dt is None:
            dt = pd.Series([fill_dt]*len(df))
        else:
    ...
    body = (df[c_body].astype(str) if c_body else pd.Series([""]*len(df)))
    url  = (df[c_url].astype(str)  if c_url  else pd.Series([""]*len(df)))
    pdf  = (df[c_pdf].astype(str)  if c_pdf  else pd.Series([""]*len(df)))
    ...
    df2 = pd.DataFrame({
        ...
        "body": body[keep].astype(str),
        "url_detail": url[keep].astype(str),
        "url_pdf": pdf[keep].astype(str),
    })
    ...
    for _,r in df2.iterrows():
        ...
        if out.exists():  # 既に変換済み
            continue
        rec = {
            "ticker": r["ticker"], "code4": r["code4"],
            "title": r["title"], "published_at_jst": r["published_at_jst"].strftime("%Y-%m-%d %H:%M:%S"),
...
by_year = {}
for p in paths[:]:
    y = re.search(r"(20\d{2})[\\/]", str(p))
    if y:
        y=y.group(1); by_year[y]=by_year.get(y,0)+1
