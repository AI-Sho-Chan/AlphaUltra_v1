...
for tk,g in td.groupby("ticker", sort=True):
    ...
    idx=np.searchsorted(p_dates, g["date"].to_numpy("datetime64[ns]"), side="right")  # > event_day
    ok=idx<len(p_dates)
    if not ok.any(): continue
    g2=g.loc[ok].copy()
    g2["eff_date"]=pd.to_datetime(p_dates[idx[ok]])
    g2=g2.merge(p.rename(columns={"date":"eff_date"}), on=["ticker","eff_date"], how="left")
    panels.append(g2)
panel=(pd.concat(panels, ignore_index=True) if panels else
        pd.DataFrame(columns=["ticker","date","eff_date","event_strength","novelty","tone_pos","tone_neg","tone_unc","event_cat","adj_close","volume"]))
panel=panel[["ticker","date","eff_date","event_strength","novelty","tone_pos","tone_neg","tone_unc","event_cat","adj_close","volume"]]\
        .dropna(subset=["adj_close"]).sort_values(["ticker","date"]).drop_duplicates(["ticker","date","event_cat"])
panel.to_parquet(OUT, index=False)
print(json.dumps({"panel_rows":len(panel),"tickers":panel["ticker"].nunique(),
                  "price_coverage":float(panel["adj_close"].notna().mean())}, ensure_ascii=False))
