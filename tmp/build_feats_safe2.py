...
# 1) 直前営業日にスナップ（per ticker, searchsorted）
maps=[]
...
def build(minp:int):
    ...
    # 基本キーを整える
    m = m.rename(columns={"eff_date_px":"eff_date"}).drop(columns=[c for c in ["eff_date"] if c in px_feat.columns], errors="ignore")
    # fe情報を戻す（ticker,eff_dateでm:1）
    m = m.merge(fe_clean.drop(columns=["eff_date"], errors="ignore"),
                on=["ticker"], how="left").drop_duplicates(subset=["ticker","eff_date"])
    # 必要列を元名へ
