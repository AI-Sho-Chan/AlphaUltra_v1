...
# --- snap to prior trading day (searchsorted) ---
maps=[]
...
# fe側の同名特徴は衝突回避で除去
fe_clean=fe.drop(columns=[c for c in NEED+["ret_1"] if c in fe.columns], errors="ignore")
...
def build(minp:int):
    ...
    px_feat = p[["ticker","eff_date"]+NEED].dropna(subset=NEED).drop_duplicates(["ticker","eff_date"])
    px_feat = px_feat.rename(columns={c:f"_fx_{c}" for c in NEED})

    m = snap.merge(px_feat, left_on=["ticker","eff_date_px"], right_on=["ticker","eff_date"],
                   how="inner", validate="m:1")

    # ここでスナップ日を正規キーに固定
    m["eff_date"] = m["eff_date_px"]
    m = m.drop(columns=[c for c in ["eff_date_px"] if c in m.columns], errors="ignore")

    # fe情報を戻す（ticker,eff_dateで m:1）
    m = m.merge(fe_clean.drop(columns=["eff_date"], errors="ignore"),
                on=["ticker"], how="left")

    # fx_元名
    have=[f"_fx_{c}" for c in NEED if f"_fx_{c}" in m.columns]
    if not have: return pd.DataFrame()
    m = m.rename(columns={f"_fx_{c}":c for c in NEED if f"_fx_{c}" in m.columns})
    ...
    # キー重複の安全除去（存在する列だけを指定）
    subset=[c for c in ["ticker","eff_date"] if c in m.columns]
    if subset: m = m.drop_duplicates(subset=subset)
...
for mp in (20,10,5):
    m=build(mp)
    if len(m)>0: break
if len(m)==0: raise SystemExit({"error":"no rows after feature build"})
