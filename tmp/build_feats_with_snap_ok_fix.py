...
def build(minp):
    ...
    # 安全リネーム
    if "eff_date" in m.columns:
        m=m.rename(columns={"eff_date":"eff_date_right"})
    m=m.rename(columns={"eff_date_px":"eff_date"})
    drop_cols=[c for c in ["eff_date_right"] if c in m.columns]
    if drop_cols: m=m.drop(columns=drop_cols)
    return m, need
