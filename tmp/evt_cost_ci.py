...
thr=0.205; H=60; Ns=[10,20]; BPS=[0.001,0.002,0.003]  # 片道コスト
...
for N in Ns:
    ...
    if pick.empty: 
        out[f"N{N}"]={"ann":0,"ci":[0,0]}; continue
    ...
    for c in BPS:
        ...
        out[f"N{N}_bps{int(c*1e4)}"]={"n":int(len(day_c)),"ann":float(ann),"ci":[float(np.percentile(bs,2.5)),float(np.percentile(bs,97.5))]}
pd.DataFrame(out).T.to_csv(os.path.join(EXP,"evt_cost_ci.csv"))
print({"evt_cost_ci":os.path.join(EXP,"evt_cost_ci.csv")})
