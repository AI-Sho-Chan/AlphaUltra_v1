...
FE="tmp/features_fnd_2600.parquet"; OUT="reports/checks/fnd_y_2x_oof.parquet"
...
if pos>0 and neg>0 and X.shape[1]>0:
  ...
  for tr,va in skf.split(X,y):
    clf=RandomForestClassifier(n_estimators=500,min_samples_leaf=5,class_weight="balanced",n_jobs=1,random_state=42)
...
print({"rows":len(y),"pos":pos,"neg":neg,"n_features":int(X.shape[1]),"p_max":float(oof.max())})
