...
df=pd.read_parquet("tmp/features_fnd_2600.parquet")
...
oof=np.full(len(y),0.01,dtype="float32"); pos,neg=int((y==1).sum()),int((y==0).sum())
if pos>0 and neg>0 and X.shape[1]>0:
  skf=StratifiedKFold(n_splits=5,shuffle=True,random_state=42)
  for tr,va in skf.split(X,y):
    clf=RandomForestClassifier(n_estimators=500,min_samples_leaf=5,class_weight="balanced",random_state=42,n_jobs=1)
...
pd.DataFrame({"ticker":df["ticker"].astype(str),"eff_date":pd.to_datetime(df["eff_date"]).dt.normalize(),
              "y":y,"p_raw":oof}).to_parquet("reports/checks/fnd_y_2x_oof.parquet",index=False)
