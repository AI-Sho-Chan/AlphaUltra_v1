...
if pos>0 and neg>0:
    ...
    skf = StratifiedKFold(n_splits=5 if len(y)>=50 else 2, shuffle=True, random_state=42)
    for tr, va in skf.split(X, y):
        dtr = lgb.Dataset(X.iloc[tr], label=y.iloc[tr])
        dva = lgb.Dataset(X.iloc[va], label=y.iloc[va])
        params = {
            "objective":"binary","metric":"auc","verbosity":-1,
            "scale_pos_weight": spw, "min_data_in_leaf": 10,
            "feature_fraction": 0.8, "bagging_fraction": 0.8, "bagging_freq": 1,
            "learning_rate": 0.05
        }
        bst = lgb.train(
            params, dtr, num_boost_round=200, valid_sets=[dva],
            callbacks=[lgb.log_evaluation(period=0), lgb.early_stopping(stopping_rounds=20)]
        )
        oof[va] = bst.predict(X.iloc[va], num_iteration=bst.best_iteration or 200)
...
print({"rows": len(oof_df), "pos": pos, "neg": neg, "auc": auc,
       "score_min": float(oof.min()), "score_max": float(oof.max())})
