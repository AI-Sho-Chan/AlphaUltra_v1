...
evt=pd.read_parquet(r"reports/checks/tdnet_model_y_2x_oof.parquet")[["ticker","eff_date","y","p_raw"]].rename(columns={"p_raw":"p_evt"})
fnd=pd.read_parquet(r"reports/checks/fnd_y_2x_oof.parquet")[["ticker","eff_date","p_raw"]].rename(columns={"p_raw":"p_fnd"})
m=evt.merge(fnd,on=["ticker","eff_date"],how="left",validate="1:1")
m["p_fnd"]=m["p_fnd"].fillna(0.01)
...
out=r"reports/checks/meta_y_2x_oof.parquet"; pathlib.Path(out).parent.mkdir(parents=True, exist_ok=True)
...
print({"rows":len(y),"pmax":float(oof.max())})
